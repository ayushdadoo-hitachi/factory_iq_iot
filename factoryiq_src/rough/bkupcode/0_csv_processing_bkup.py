...
required_columns_df = spark.table(REQUIRED_STAGE0_COLUMNS_TABLE)
...
# MAGIC %skip
# MAGIC def read_and_prepare(path):
# MAGIC     # Read raw CSV as text
# MAGIC     raw_df = spark.read.text(path)
# MAGIC
# MAGIC     # Add a row number
# MAGIC     df_rn = raw_df.withColumn(
# MAGIC         "rn",
# MAGIC         F.row_number().over(Window.orderBy(F.monotonically_increasing_id()))
# MAGIC     )
# MAGIC
# MAGIC     # print("hello1")
# MAGIC
# MAGIC     # Get header row (assume second row has header)
# MAGIC     header_row = df_rn.filter(F.col("rn") == 2).select("value").first()[0]
# MAGIC
# MAGIC     # Remove trailing semicolon if exists
# MAGIC     header_row = header_row.rstrip(";")
# MAGIC
# MAGIC     new_columns = header_row.split(";")
# MAGIC     # print("hello2")
# MAGIC     print(f"Number of header columns: {len(new_columns)}")
# MAGIC     # print(new_columns)
# MAGIC
# MAGIC     # Extract data rows (from row 4 onward)
# MAGIC     data_only = df_rn.filter(F.col("rn") >= 4).select("value")
# MAGIC     # print("hello3")
# MAGIC
# MAGIC     # Split the value into array
# MAGIC     arr = F.split(F.col("value"), ";")
# MAGIC
# MAGIC     # Find maximum number of fields in ANY data row
# MAGIC     max_fields = data_only.select(F.max(F.size(arr))).first()[0]
# MAGIC     print(f"Max fields in data rows: {max_fields}")
# MAGIC
# MAGIC     # Choose safe limit
# MAGIC     limit = min(len(new_columns), max_fields)
# MAGIC     # print(f"Building dataframe with {limit} columns safely.")
# MAGIC
# MAGIC     # Use get() for safe array access
# MAGIC     final_df = data_only.select(
# MAGIC         *[F.expr(f"get(split(value,';'), {i}) as `{new_columns[i]}`") for i in range(limit)]
# MAGIC     )
# MAGIC
...
# ✅ verify
# show_columns(df_raw, "After deduplication")
