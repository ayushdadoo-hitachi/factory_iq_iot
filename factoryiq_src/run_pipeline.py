...
processed = bronze.run_stage(
    ...
    limit_files=None,                   # or an int during dev, e.g., 5
    delta_options={"mergeSchema": "true"},
    add_batch_id=True,                  # keep traceability
    use_manifest=True,                  # skip already processed files
    manifest_path=f"{config_paths.bronze_path}_manifest",
    move_to_date_partition=True,        # archive/dt=YYYY-MM-DD
    dbutils_=dbutils                    # pass dbutils when calling from a notebook
)
...
# MAGIC # ==========================================
# MAGIC processed = bronze.run_stage(
# MAGIC     spark,
# MAGIC     inflow_dir=config_paths.inflow_dir,
# MAGIC     bronze_path=config_paths.bronze_path,
# MAGIC     archive_dir=config_paths.archive_dir,
# MAGIC     # limit_files=5,                     # dev: only first 5
# MAGIC     add_batch_id=True,                 # traceability
# MAGIC     use_manifest=True,                 # avoid duplicates
# MAGIC     manifest_path=f"{config_paths.bronze_path}_manifest",
# MAGIC     move_to_date_partition=True        # organize archive under dt=YYYY-MM-DD
# MAGIC )
# MAGIC
# MAGIC logger.info(f"Bronze Processed files:\n{'\n'.join(processed)}")
...
# Silver (unchanged call signature; now batch-aware)
...
logger.info(f"Silver row sample count: {silver_df.count()}")
...
# MAGIC # ==========================================
# MAGIC
# MAGIC silver_df = silver.run_stage(
# MAGIC     spark=spark,
# MAGIC     input_path=config_paths.bronze_path,
# MAGIC     ref_weld_path=config_paths.weld_features_path,
# MAGIC     output_path=config_paths.silver_path
# MAGIC )
# MAGIC
# MAGIC logger.info(f"Silver Stage row count = {silver_df.count()}")
# MAGIC
# MAGIC # display(silver_df.limit(10))
...
gold_df = gold.run_stage(
    ...
    wfs_unit="auto",
    write_mode="append",  # keep append for incremental
...
# MAGIC %skip
# MAGIC # ==========================================
# MAGIC # gold - data enrichment
# MAGIC # ==========================================
# MAGIC
# MAGIC gold_df = gold.run_stage(
# MAGIC     spark=spark,
# MAGIC     input_path=config_paths.silver_path,
# MAGIC     wps_ref_path=config_paths.wps_path,
# MAGIC     tps500current=config_paths.tps500current,
# MAGIC     techdevisrunning=config_paths.techdevisrunning,
# MAGIC     output_path=config_paths.gold_path,
# MAGIC
# MAGIC     enable_wire_consumption=True,  # default True; keep explicit
# MAGIC     wire_episodes_output_path=config_paths.gold_wirefeed_episodes_path,
# MAGIC
# MAGIC     time_col=config_paths.time_col,
# MAGIC     wfs_col=config_paths.wfs_col,
# MAGIC     wfs_unit="auto",
# MAGIC     write_mode="overwrite",
# MAGIC     round_wire_outputs=True,
# MAGIC     round_digits=2,
# MAGIC     log_counts=False,
# MAGIC )
...
spark.read.format("delta").load(config_paths.silver_path).printSchema()

# COMMAND ----------

spark.read.format("delta").load(config_paths.silver_path) \
     .select("batch_id").distinct().show()

# COMMAND ----------

dbutils.fs.rm(config_paths.silver_path, recurse=True)
dbutils.fs.rm(config_paths.silver_path + "_manifest", recurse=True)
